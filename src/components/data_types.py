...
class Utterance(BaseModel):
    role: str = Field(..., description="The role of the utterance, e.g., 'user', 'assistant', 'system'")
...
class Messages(BaseModel):
    utterances: List[Utterance] = Field(default_factory=list, description="List of utterances in the conversation")
    ...
    def __repr__(self):
        return "\n".join([f"{utterance.role}: {utterance.content}" for utterance in self.utterances])