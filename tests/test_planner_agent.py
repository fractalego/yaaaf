...
from yaaaf.components.data_types import (
    Messages,
    Utterance,
    AgentTaxonomy,
    DataFlow,
...
This workflow shows the complete data pipeline from extraction to visualization.
        ...
        messages = Messages(
            utterances=[
                Utterance(role="user", content="Create a sales visualization workflow")
            ]
        )
...
"""
    ...
    def test_agent_descriptions_include_taxonomy(self):
        ...
        descriptions = self.planner_agent._create_agent_descriptions(
            self.available_agents
        )
