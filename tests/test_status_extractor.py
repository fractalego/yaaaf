...
class TestStatusExtractor(unittest.TestCase):
    ...
    def test_get_current_step_info(self):
        ...
        self.assertEqual(step_info['current_step_index'], 2)  # First pending task (1-indexed)
    ...
    def test_generate_markdown_todo_list(self):
        ...
        markdown = self.extractor._generate_markdown_todo_list(self.sample_df, 1)  # Index 1 is current
    ...
    def test_fallback_status_evaluation(self):
        ...
        self.assertEqual(self.extractor._fallback_status_evaluation("Task completed <taskcompleted/>", "pending"), "completed")
        self.assertEqual(self.extractor._fallback_status_evaluation("The task is complete.", "pending"), "completed")
        self.assertEqual(self.extractor._fallback_status_evaluation("Working on it...", "pending"), "in_progress")
        self.assertEqual(self.extractor._fallback_status_evaluation("", "pending"), "pending")

    def test_has_meaningful_content(self):
        """Test meaningful content detection."""
        self.assertTrue(self.extractor._has_meaningful_content("This is a substantial response with content."))
