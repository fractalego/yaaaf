...
class URLAgent(BaseAgent):
    ...
    async def query(
        ...
        if not url or not instruction:
            return (
                "Could not parse URL and instruction from the input. "
                "Please provide the URL and instruction in the format:\n"
                "URL_HERE | INSTRUCTION_HERE"
            )

        for step_idx in range(self._max_steps):
            content = self._fetch_url_content(url)
            new_messages = Messages().add_system_prompt(
